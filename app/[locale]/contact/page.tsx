...
export default async function ContactPage({ params, searchParams }: ContactPageProps) {
  ...
  const selectedModels = (query.models ?? "")
    .split(",")
    .map((model) => model.trim())
    .filter(Boolean);
  const initialMessage = selectedModels.length
    ? `${locale === "zh-cn" ? "询盘型号" : "Inquiry models"}: ${selectedModels.join(", ")}`
    : "";
    ...
    <Container className="py-16">
      ...
      <div className="mt-12 grid gap-8 lg:grid-cols-[0.85fr_1.15fr]">
        ...
        <div className="rounded-[2rem] border border-line bg-[#e9ede4] p-8">
          <ContactFormClient locale={locale} copy={copy.contact.form} initialMessage={initialMessage} />
        </div>
      </div>
    </Container>
  );
