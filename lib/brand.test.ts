...
test("website identity uses EKD and the confirmed company display name", () => {
  ...
  assert.equal(brand.companyEnglish, "Jiangsu EKD Machinery Technical Co., Ltd.");
...
for (const locale of locales) {
  test(`${locale} company copy and article publisher use the same identity`, () => {
    ...
    assert.ok(paragraphs.includes(getBrandCompanyName(locale)));
    assert.ok(paragraphs.includes(brand.name));
    assert.doesNotMatch(JSON.stringify(copy), /OVICTOR|奥维达|亿凯达/i);
    assert.doesNotMatch(JSON.stringify(copy.home.trustSection), /30\+|15\+|ISO9001|ROHS|\bCE\b/);
    ...
    assert.equal(publisher.name, brand.companyEnglish);
