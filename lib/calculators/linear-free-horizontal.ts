...
export const linearFreeHorizontalCalculator: ScenarioCalculator<Input> = {
  ...
  calculate: (input) => {
    ...
    const averageDampingForceN = totalEnergyPerCycleNm / criticalStrokeM;
    ...
    return {
      ...
      averageImpactForceN: roundTo(averageDampingForceN, 3),
      ...
      detailMetrics: [
        ...
        { key: "averageDampingForceN", value: roundTo(averageDampingForceN, 3), unit: "N" },
  ...
  explain: (input, result) => [
    ...
    `This path uses a fixed critical stroke of ${result.requiredStrokeMm} mm and estimates ${result.averageImpactForceN} N average damping force; external thrust is not used.`,
  ],
};
