...
export async function HomePage({ locale }: HomePageProps) {
  ...
  let catalogFamilies: Awaited<ReturnType<typeof findCatalogFamilies>> = [];
  let hasLiveCatalog = true;

  try {
    const [catalogCount, families] = await Promise.all([
        ...
        <Container className="py-12">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            {scenarioCategories.map((item) => (
              <Link
                key={item.key}
                href={getLocalizedHref(locale, `/selector/engineer?entryKey=${item.key}`)}
      ...
      <section className="section-border">
        <Container className="py-20">
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {copy.home.applicationSection.sectors.map((sector, index) => (
              <Link
                key={sector.title}
                href={getLocalizedHref(locale, applicationHrefs[index] ?? sector.href)}
