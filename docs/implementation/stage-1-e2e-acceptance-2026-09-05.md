...
2. Application notification SMTP credentials are stored in an ignored owner-only local file. ZeptoMail customer validation was submitted on 2026-09-05. A ZeptoMail web-console test was sent and delivered, but the local application SMTP test did not enter ZeptoMail: local DNS returned a non-public `198.18.0.0/15` address and direct connections to the real SMTP IP on both 587 and 465 closed before TLS. SMTP acceptance testing therefore remains incomplete. The hidden accessibility-DOM text `This Agent has been shutdown` is not visible in the rendered UI and must not be treated as the Agent's authoritative status.
