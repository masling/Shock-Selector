...
2. ZeptoMail customer validation was submitted on 2026-09-05, and a web-console test was sent and delivered. The local application SMTP test did not enter ZeptoMail because outbound 587/465 connections closed before TLS, so application notifications now use ZeptoMail's HTTPS API instead. The Agent Send Mail Token is stored only in ignored owner-only local configuration. A controlled no-customer-data API test returned `EM_104`; the ZeptoMail dashboard increased from Sent/Delivered 1/1 to 2/2 with zero process failures, and the user confirmed the message arrived in the destination inbox. Supabase Auth keeps its separate cloud SMTP configuration. The hidden accessibility-DOM text `This Agent has been shutdown` is not visible in the rendered UI and must not be treated as the Agent's authoritative status.
